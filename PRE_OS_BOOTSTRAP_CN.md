...
- 首启自动克隆顶层 bring-up 仓库并初始化 `lelamp_runtime` submodule
...
如果你是手工 clone 顶层仓库到另一台机器上先检查内容，记得第一次就用：

```bash
git clone --recurse-submodules <你的 Lelamp 仓库 URL> Lelamp
```

已有旧 checkout / worktree 则执行：
...
3. 克隆顶层 bring-up 仓库并初始化 `lelamp_runtime` submodule
...
如果这台 Pi 之前已经拉过旧版本顶层仓库，但没有带 submodule，可以手工补一遍：
