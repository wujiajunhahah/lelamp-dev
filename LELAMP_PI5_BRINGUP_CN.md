...
如果你已经有旧 checkout、旧 worktree，或者之前只拉了顶层仓库但没有 runtime：

```bash
git submodule update --init --recursive
```

如果你按本文默认零接触脚本在 Pi 上部署，顶层仓库默认落在 `~/lelamp-dev`，所以运行时目录通常是 `~/lelamp-dev/lelamp_runtime`。
...
4. 如果你要手工重跑或补跑，再在 Pi 上进入运行时目录执行：
