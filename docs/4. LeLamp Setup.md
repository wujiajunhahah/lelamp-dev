...
Then set up the maintained runtime from this repository:
...
If you are iterating from a Mac development machine later, prefer syncing
updates with:
...
That keeps the Pi's local `.env` and `.venv` while refreshing code, tests and
dashboard.
