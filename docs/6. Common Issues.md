...
- **Use the maintained repo layout**: Clone `wujiajunhahah/lelamp-dev` and work
  inside `lelamp_runtime/`, instead of mixing this repo with the old upstream
  runtime checkout

**Problem**: Phone or laptop cannot open the local dashboard

- **Check `reachable_urls`**: Run `curl -s http://127.0.0.1:8765/api/state`
  on the Pi and read the advertised URLs
- **Check LAN / hotspot**: Ensure the Pi and your phone are on the same Wi-Fi or
  hotspot network
- **Restart via sync script**: From `lelamp_runtime/`, use
  `START_DASHBOARD=1 ./scripts/sync_pi_runtime.sh`
- **Check bind address**: Dashboard should listen on `0.0.0.0:8765`
...
- **GitHub Issues**: https://github.com/wujiajunhahah/lelamp-dev/issues
- **Discord Community**: https://discord.gg/4hmNW3Ep
- **Local Runtime Issues**: https://github.com/wujiajunhahah/lelamp-dev/issues
