...
`lelamp_runtime/` 现在按 submodule 管理，表达的是“顶层仓库固定引用一份 runtime 版本”，而不是假定每个维护者本地另外 clone 一份隐式依赖。

维护时注意：

1. 首次 clone 顶层仓库，要带上 `--recurse-submodules`
2. 旧 checkout / 旧 worktree 拉到新版本后，要执行 `git submodule update --init --recursive`
3. 如果 runtime 本身要更新，先在 `lelamp_runtime/` 内完成变更、提交和推送，再回到顶层仓库更新 submodule 指针
4. 不要再把 runtime 当成顶层仓库之外的一份“默认存在”的目录
...
默认值是 `Qwen Omni Realtime`：

- provider: `qwen`
- base URL: `https://dashscope.aliyuncs.com/api-ws/v1/realtime`
...
1. 如果涉及 runtime，先在 `lelamp_runtime/` 内改脚本或配置
2. 回到顶层仓库更新 submodule 指针
3. 再更新 `README.md`
4. 再更新 `site/index.html` / `site/developer.html`
5. 再更新中文说明文档
6. 最后做 shell 语法检查和 Python 编译检查
